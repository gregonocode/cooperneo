...
class _RelatoriosScreenState extends State<RelatoriosScreen>
  ...
  Widget build(BuildContext context) {
    final relatoriosViewModel = Provider.of<RelatoriosViewModel>(context);

    return Scaffold(
      appBar: AppBar(
        title: const Text('Relatórios'),
      ),
      body: relatoriosViewModel.isLoading
  ...
  void _gerarRelatorioDiario(BuildContext context) async {
    ...
    try {
      final arquivo = await viewModel.gerarRelatorioDiarioPDF(hoje);
      Navigator.pop(context); // Fechar dialog de carregamento

      if (arquivo != null) {
        _showReportOptionsDialog(context, arquivo);
      } else {
        _showErrorDialog('Não foi possível gerar o relatório diário.');
      }
    } catch (e) {
      Navigator.pop(context); // Fechar dialog de carregamento
  ...
  void _gerarRelatorioSemanal(BuildContext context) async {
    ...
    try {
      final arquivo = await viewModel.gerarRelatorioSemanalPDF(hoje);
      Navigator.pop(context); // Fechar dialog de carregamento

      if (arquivo != null) {
        _showReportOptionsDialog(context, arquivo);
      } else {
        _showErrorDialog('Não foi possível gerar o relatório semanal.');
      }
    } catch (e) {
      Navigator.pop(context); // Fechar dialog de carregamento
      _showErrorDialog('Erro ao gerar relatório: $e');
  ...
  void _gerarRelatorioPeriodo(BuildContext context) async {
    ...
    try {
      final arquivo = await viewModel.gerarRelatorioPersonalizadoPDF(
        _dataInicio,
        _dataFim,
      ...
      if (arquivo != null) {
        ...
        _showErrorDialog('Não foi possível gerar o relatório personalizado.');
      }
    } catch (e) {
      Navigator.pop(context); // Fechar dialog de carregamento
      _showErrorDialog('Erro ao gerar relatório: $e');
  ...
  void _gerarRelatorioEstoque(BuildContext context) async {
    ...
    try {
      final arquivo = await viewModel.gerarRelatorioEstoquePDF();
      Navigator.pop(context); // Fechar dialog de carregamento

      if (arquivo != null) {
        _showReportOptionsDialog(context, arquivo);
      } else {
        _showErrorDialog('Não foi possível gerar o relatório de estoque.');
      }
    } catch (e) {
      Navigator.pop(context); // Fechar dialog de carregamento
      _showErrorDialog('Erro ao gerar relatório: $e');
  ...
  void _showReportOptionsDialog(BuildContext context, File arquivo) {
    final viewModel = Provider.of<RelatoriosViewModel>(context, listen: false);
                ...
                children: [
                  _buildOptionButton(
                    ...
                    onPressed: () {
                      ...
                      viewModel.visualizarPDF(arquivo);
                    },
                  ),
                  _buildOptionButton(
                    label: 'Compartilhar',
                    ...
                    onPressed: () {
                      Navigator.pop(context);
                      viewModel.compartilharPDF(arquivo);
                    },
                  ),
                ],
              ),
