...
class _FornecedoresScreenState extends State<FornecedoresScreen> {
  ...
  Widget _buildFornecedoresList(EstoqueViewModel viewModel) {
    final List<Fornecedor> filteredFornecedores = viewModel.fornecedores
        .where((f) =>
        ...
        .toList()
      ..sort(
          (a, b) => (a.nome ?? '').compareTo(b.nome ?? '')); // Ordenar por nome
  ...
  Widget _buildFornecedorItem(
      Fornecedor fornecedor, EstoqueViewModel viewModel) {
    return Slidable(
  ...
  void _showFornecedorDetailsDialog(
      BuildContext context, Fornecedor fornecedor) {
    showDialog(
      context: context,
                  ...
                  ElevatedButton.icon(
                    onPressed: () {
                      ...
                      _showDeleteConfirmationDialog(
                        ...
                        'Excluir Fornecedor',
                        'Deseja realmente excluir o fornecedor "${fornecedor.nome ?? 'Desconhecido'}"? Esta ação não pode ser desfeita.',
                        () => Provider.of<EstoqueViewModel>(context,
                                listen: false)
